...
interface BookAppointmentProps {
  ...
  onBook?: (booking: { doctorId: number; dateKey: string; slot: string }) => void
}
...
export default function BookAppointment({ onBack, doctor, bookedSlots = {}, onBook }: BookAppointmentProps) {
  ...
  const selectedDateKey = getDateKey(dates[selectedDate].full)
  const bookedSlotsForDate = new Set(bookedSlots[selectedDateKey] ?? [])

  const handleBook = async () => {
    if (!selectedSlot || bookedSlotsForDate.has(selectedSlot)) return
    setLoading(true)
              ...
              <div className="grid grid-cols-3 gap-2">
                {SLOTS.map(slot => {
                  ...
                  <button
                    ...
                    style={{
                      background: isBooked ? '#f3f4f6' : isSelected ? 'var(--cyan)' : '#f9fafb',
                      border: `1.5px solid ${isBooked ? '#e5e7eb' : isSelected ? 'var(--cyan)' : 'var(--border)'}`,
